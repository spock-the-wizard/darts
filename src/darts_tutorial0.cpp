...
void test_vectors_and_matrices()
{
    ...
    float degrees = Spherical::rad2deg(angle(v1,v3));
    // float degrees = Spherical::rad2deg(acos(dot(v1,v3)));
...
void test_color_and_image()
{
    ...
    Color3f pinkish = 0.5f * (red + white);
    ...
    spdlog::info("Green channel of pinkish is: {}", pinkish[1]);
    ...
    for (int i = 0; i < gradient.width(); i++) {
        for (int j=0; j < gradient.height(); j++){
            float p_w = (float)i / gradient.width();
            float p_h = (float)j / gradient.height();
            gradient.at(i,j) = Color3f(lerp(0.0f,1.0f,p_w),lerp(0.0,1.0f,p_h),0.0f);
        }
        }   
    ...
    for (int i=0;i<image.width();i++){
        for(int j=0;j<image.height();j++){
            image.at(i,j) = Color3f(luminance(image.at(i,j)));
        }
    }
    ...
    image.save("scenes/assignment0/cornellbox_grayscale.png");
